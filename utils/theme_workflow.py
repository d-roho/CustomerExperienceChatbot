...
async def generate_preliminary_themes(state: State, llm_handler: LLMHandler, vector_store: VectorStore, index: str) -> State:
    ...
    try:
        response = llm_handler.anthropic.messages.create(
            ...
            max_tokens=4000,
            temperature=0,
            system="""You are tasked with performing a thematic analysis on a set of reviews for a jewelry store. Your goal is to identify main themes and subthemes within these reviews. Return only valid JSON. Here are your instructions:
...
{{"refined_themes": [theme1, theme2,.....]}}""",
            messages=[{
                "role": "user",
                "content": f"""Based on the reviews provided, here is my thematic analysis:
