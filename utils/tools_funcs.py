...
import uuid
import copy
from utils.vector_funcs import get_unix_time
    ...
    for key in filter.keys():
        ...
        if key == 'cities':
            if filter[key]:
                ...
                valid_cities = [
                    str(city) for city in filter[key] if city is not None
                ]
                if valid_cities:
        ...
        if key == 'location':
            if filter[key]:
                dict = {"name": 'Place Location', "values": filter[key]}
                filters.append(dict)
                filters_exist = 1
        ...
        if key == 'month_start':
            ...
            start_date = get_unix_time(ms, ys, 1)
            end_date = get_unix_time(me, ye, 28)
            dict = {
                "name": 'Date Created',
                "minimum": start_date,
                "maximum": end_date
...
def sort_and_select_sentiment(df: pd.DataFrame, cols: list,
                              n: int) -> (pd.DataFrame):
    # Sort the DataFrame by two columns
    ...
    mid_n = df.sort_values(by=[cols[0]],
                           ascending=False).iloc[(row_count // 2) -
                                                 (n // 2):(row_count // 2) +
                                                 (n // 2)]
...
def sentiments_processing(result, api_start_time):
    ...
    final_df = sort_and_select_sentiment(
        filtered_df,
        ['Proportion of Positive Mentions', 'Proportion of Negative Mentions'],
        15)
...
async def process_combinations(subset_combinations, filter, filters,
        ...
        if sub in [
            ...
            for diction in filters:
                if diction['name'] == 'Date Created':
                    diction['minimum'] = get_unix_time(1, combo[idx], 1)
                    diction['maximum'] = get_unix_time(12, combo[idx], 28)
                    idx += 1
